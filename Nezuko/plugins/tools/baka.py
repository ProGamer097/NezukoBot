...
@app.on_message(filters.command("baka"))
async def es_img(_, message):
    ...
    try:
        ...
        if image:
            sent_message = await message.reply_video(video=image, caption="BY @Nezuko_RoxBot", parse_mode=None)
            asyncio.create_task(delete_message_after_timeout(sent_message, 5))  # Delete after 1 minute
        else:
